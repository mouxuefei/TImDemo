package com.edocyun.timchat.imageEngine.impl;
...
import com.edocyun.timchat.imageEngine.ImageEngine;
...
public class GlideEngine implements ImageEngine {
    ...
    public static void loadCornerImage(ImageView imageView, String filePath, RequestListener listener, float radius) {
        CornerTransform transform = new CornerTransform(TUIConfig.getAppContext(), radius);
        RequestOptions options = new RequestOptions()
                .centerCrop()
                .placeholder(R.mipmap.ic_launcher)
                .transform(transform);
        Glide.with(TUIConfig.getAppContext())
                .load(filePath)
                .apply(options)
                .listener(listener)
                .into(imageView);
    ...
    public static void clear(ImageView imageView) {
        Glide.with(TUIConfig.getAppContext()).clear(imageView);
    }
    ...
    public static void loadImage(String filePath, String url) {
        try {
            File file = Glide.with(TUIConfig.getAppContext()).asFile().load(url).submit().get();
    ...
    public static void loadUserIcon(ImageView imageView, Object uri, int defaultResId, int radius) {
        ...
        Glide.with(TUIConfig.getAppContext())
                .load(uri)
                .apply(new RequestOptions().centerCrop().error(defaultResId).transform(transform))
                .into(imageView);
    }
    ...
    public static Bitmap loadBitmap(Object imageUrl, int targetImageSize) throws InterruptedException, ExecutionException {
        ...
        return Glide.with(TUIConfig.getAppContext()).asBitmap()
                .load(imageUrl)
                .apply(new RequestOptions().error(R.mipmap.ic_launcher))
